...
contract NFTPoolBurnAndMint is CCIPReceiver, OwnerIsCreator {
    ...
    // Event emitted when a message is received from another chain. 解锁
    event TokenUnlocked(
        uint256 tokenId,
        address newOwner
    );
    ...
    WrappedMyToken public wnft; // The NFT contract that will be used to lock and send NFTs.
    ...
    /// @param _router The address of the router contract. ccip 库里的本地安装模拟环境 
    /// @param _link The address of the link contract. ccip 库里的 本地安装模拟环境
    constructor(address _router, address _link, address _wnf) CCIPReceiver(_router) {
        wnft = WrappedMyToken(_wnf);
    ...
    /// @param payload The text to be sent.
    ...
    //    tokenLocked[tokenId] = true; // 标记NFT已被锁定
    //    return messageId;
